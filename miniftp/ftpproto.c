...
static void do_size(session_t *sess)
{
	...
	if(!S_ISREG(sbuf.st_mode))//判断是否是普通文件
	{
		ftp_reply(sess, FTP_FILEFAIL, "Could not get file size.");
		return;
	}
...
static void do_stor(session_t *sess)
{
	...
	int fd = open(sess->arg,  O_CREAT|O_WRONLY, 0755);//创建/打开文件
	if(fd == -1)
	{
	...
	sess->restart_pos = 0;
	if(lseek(fd, offset, SEEK_SET) < 0)//偏移出现错误
	{
	...
	while(1)
	{
		...
		if(write(fd, buf, ret) != ret)//写入数据到文件
		{
...
static void do_rest(session_t *sess)
{
	sess->restart_pos = (long long)atoll(sess->arg);//记录偏移量
