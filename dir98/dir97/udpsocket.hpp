...
class UdpSocket
{
    public:
        ...
        bool Recv(string* buf, string *ip=NULL, uint16_t *port=NULL)//接收数据，获取发送端地址信息
        {
            //recvfrom(套接字，接收缓冲区，数据长度，标志(0阻塞标志)，源端地址(输入输出型数据)，地址长度)
            struct sockaddr_in source_addr;
	    char tmp[4096]={0};
	    socklen_t len = sizeof(struct sockaddr_in);
            int ret = recvfrom(_sockfd, tmp,4096,0,(struct sockaddr*)&source_addr,&len);
            if (ret < 0 )
            {
	    ...
	    if (port)
            ...
            buf->assign(tmp,ret);//从指定字符串中截取指定长度放到buf中
